...
public class KingbaseDriver extends AbstractDriver {
    ...
    public void create(DriverModeEnum driverMode, ConfigProperties properties) throws Exception {
        ...
        dataSource.setDriverClassName("com.kingbase8.Driver"); // 设置连接池的数据库驱动
        dataSource.setUrl(String.format("jdbc:kingbase8://%s:%s/%s",
    ...
    public boolean test(ConfigProperties properties) {
        ...
        try {
            ...
            DriverManager.getConnection(String.format("jdbc:kingbase8://%s:%s/%s",
                    properties.getString("ip"),
                    properties.getString("port"),
            ...
            Loggers.DRIVER.error("kingbase driver test {}", e.getMessage());
            return false;
        }
    }
    ...
    public void scheduleTrigger(ConfigProperties properties) throws Exception {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            Loggers.DRIVER.error("kingbase driver error {}", e.getMessage());
            throw e;
        }
